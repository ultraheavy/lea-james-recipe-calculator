#!/usr/bin/env python3
"""
Import Toast POS data into restaurant calculator database (Fixed version)
"""
...
def import_vendors():
    ...
    # Skip the first 3 header rows and start from the actual data
    with open(item_file, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            if vendor_name and vendor_name != 'Vendor Name' and len(vendor_name) > 1:
        ...
        with get_db() as conn:
            for row in reader:
                try:
                    ...
                    if not item_description or not item_code:
                        continue
                    
                    # Parse prices safely
                    def safe_float(value):
                        try:
                            return float(value) if value and str(value).strip() else 0
                        except:
                            return 0
                    
                    current_price = safe_float(row.get('Contracted Price ($)', 0))
                    last_price = safe_float(row.get('Last Purchased Price ($)', 0))
